...
dag = DAG(
    dag_id='szcp-http-srcAddress-5m-mf', default_args=args,
    ...
    "filtered": {
      "query": {
        "query_string": {
          ...
          "query": "protocolType:HTTP AND securityEyeLogType:1 AND srcAddress:*"
        }
      },
      "filter": {
        "bool": {
...
dest_map = {
    ...
    'attrs': {
        '_id': 'key',
        'srcAddress':'key',
        'srcAddress_num': 'doc_count'
    }
}

run_this = EsToEsTransfer(
    task_id='szcp-http-srcAddress-5m-mf', src_index='dpi-index-*', src_query=query,
	dest_map=dest_map,dest_index='szcp-speed-up-index',dest_type='http-srcAddress-5m', dag=dag)
